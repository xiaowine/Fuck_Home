 ...
 * @since 05-05-2021
 */
class Shell @Throws(NotFoundException::class) @JvmOverloads constructor(val path: String, val environment: EnvironmentMap = emptyMap()) {
     ...
     */
    constructor(shell: String, vararg environment: Pair<Variable, Value>) : this(shell, environment.toEnvironmentMap())
     ...
     *                    with the system environment.
     */
    constructor(shell: String, environment: Array<String>) : this(shell, environment.toEnvironmentMap())
     ...
     * @see run
     */
    @Throws(ClosedException::class) @Synchronized fun run(
        command: String,
     ...
     * @see run
     */
    @Throws(ClosedException::class) @Synchronized @JvmOverloads fun run(
        command: String,
    ...
    object Command {
         ...
         */
        data class Result(val stdout: List<String>, val stderr: List<String>, val output: List<String>, val exitCode: Int, val details: Details) {
             ...
             */
            data class Details internal constructor(val uuid: UUID, val command: String, val startTime: Long, val endTime: Long, val elapsed: Long = endTime - startTime)
         ...
         */
        class Config private constructor(val uuid: UUID = UUID.randomUUID(), val redirectStdErr: Boolean = false, val onStdOut: (line: String) -> Unit = {}, val onStdErr: (line: String) -> Unit = {}, val onCancelled: () -> Unit = {}, val onTimeout: () -> Unit = {}, val timeout: Timeout? = null, val notify: Boolean = true) {
            ...
            class Builder {
                 ...
                 */
                fun create() = Config(uuid, redirectErrorStream, onStdOut, onStdErr, onCancelled, onTimeout, timeout, notify)
            }
    ...
    private class Watchdog : CountDownLatch(STREAM_READER_COUNT) {
         ...
         */
        @Throws(AbortedException::class) fun await(timeout: Timeout?): Boolean {
            return when (timeout) {
                null -> {
                    await(); true
                }
     ...
     */
    private class StreamReader private constructor(name: String, private val stream: InputStream) : Thread(name) {
        ...
        override fun run() = BufferedReader(InputStreamReader(stream)).forEachLine { line ->
            pattern.matcher(line).let { matcher ->
                if (matcher.matches()) {
                    ...
                    onComplete(when (val exitCode = matcher.group(GROUP_CODE)) {
        ...
        companion object {
            ...
            private val pattern: Pattern = Pattern.compile("^([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})\\s?([0-9]{1,3})?$", Pattern.CASE_INSENSITIVE)

            internal fun createAndStart(name: String, stream: InputStream) = StreamReader(name, stream).also { reader -> reader.start() }
        }
    }
    ...
    companion object {
        ...
        private fun runWithEnv(command: String, environment: EnvironmentMap): Process = Runtime.getRuntime().exec(command, (System.getenv() + environment).toArray())
         ...
         */
        private fun Array<out String>.toEnvironmentMap(): EnvironmentMap = mutableMapOf<Variable, Value>().also { map ->
            forEach { str ->
         ...
         */
        private fun Array<out Pair<Variable, Value>>.toEnvironmentMap(): EnvironmentMap = mutableMapOf<Variable, Value>().also { map ->
         ...
         */
        private fun EnvironmentMap.toArray(): Array<String> = mutableListOf<String>().also { list ->
            forEach { (variable, value) ->
